...
def process_pdf(pdf_file):
    try:
        # Get recommendations using the job_recommender module
        recommendations = recommend_jobs_from_resume(pdf_file)
        ...
        for i, job in enumerate(recommendations, 1):
            ...
            output += f"   Required Skills: {', '.join(job['skills'])}\n"
...
demo = gr.Interface(
    fn=process_pdf,
    inputs=gr.File(file_types=[".pdf"], label="Upload your Resume (PDF)"),
    outputs=gr.Textbox(label="Job Recommendations", lines=20),
    title="Job Recommendation System",
    description="Upload your resume (PDF) to get personalized job recommendations based on your skills and experience"
)
